...
void base_logger_type::setup_logs() {
  ...
  system("grep EXITCODE .tamulauncher/log* | grep \"EXITCODE=0\" | cut -d\" \" -f1 | cut -d\":\" -f2 > .tamulauncher.processed");
  system("grep EXITCODE .tamulauncher/log* | grep -v \"EXITCODE=0\" | cut -d\" \" -f1 | cut -d\":\" -f2 > .tamulauncher.exited");
}
