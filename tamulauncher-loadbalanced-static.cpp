...
extern "C" int tamulauncher_run_function(int, char**); // one way

using namespace std;

//define the run_function
//int tamulauncher_run_function(int argc,char** argv);
...
public:
  ...
  void execute() {
    ...
    // argv has to hold one extra element; the task_id
    char* argv[argc+1];
    ...
    // add the task id as the last argument
    //char *intStr = itoa(a);
    int task_id;
    string tstring;
    MPI_Comm_rank(MPI_COMM_WORLD,&task_id);
    tstring = std::to_string(task_id);
    argv[argc] = new char(tstring.size() + 1);
    strcpy(argv[argc], tstring.c_str());
    ++argc;

    double tstart = MPI::Wtime();
    
    this->return_code = tamulauncher_run_function(argc,argv);
...
int pack_and_send(vector<string>& commands, int commands_index,
		  ...
		  int chunksize, int destination, task_info_type& task_info) {
