...
{
   # On ada, will use mpirun, so need to set the mpi env var
   ${mpi_env}
    
   # for informative reason also check if -app flag was used
   # only applies for non-interactive jobs
   jobid=$(get_job_id)

   if [ ${jobid} -ne "000" ]; then
